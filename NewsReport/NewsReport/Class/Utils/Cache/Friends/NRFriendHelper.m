...
- (void)p_initTestData
{
    ...
    NSArray *arr = [NRUser mj_objectArrayWithKeyValuesArray:jsonArray];
    [self.friendsData removeAllObjects];
    [self.friendsData addObjectsFromArray:arr];
    // 更新好友数据到数据库
    BOOL ok = [self.friendStore updateFriendsData:self.friendsData forUid:[[NRUserHelper defaultCenter]getUserID]];
    if (!ok) {
        NSLog(@"保存好友数据到数据库失败!");
    }
